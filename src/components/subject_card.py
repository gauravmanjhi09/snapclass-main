...
#             html+=f"<div style=backgound: #E0E3FF; padding: 5px 12px; border-radius: 12px; font-size:0.9rem">{icon} <b>{value}</b> {label} </div>
...
def subject_card(name, code, section, stats=None, footer_callback=None):
    ...
    if stats:
        ...
        """
        for icon, label, value in stats:
            html += f"""<div style="background:#E0E3FF; padding: 5px 12px; border-radius: 12px; font-size:0.9rem;">{icon} <b>{value}</b> {label}</div>"""
        html += "</div>"

    html += "</div>"  # closing the outer card div
